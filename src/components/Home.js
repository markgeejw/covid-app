...
export default class Home extends Component {
    ...
    render() {
        ...
        country_select_options.push({ label: "Select country", value: "", color: "grey" });
        Object.keys(supported_regions).forEach(country => {
            country_select_options.push({ label: country.includes(", ") ? 
                                        country.split(", ").reverse().join(" ") : // flipping Korea, South
                                        country, 
                                        value: country });
        })
        const country_selected = country !== "";
        const no_states = country === '' || supported_regions[country].length === 0;
        if (!no_states){
        ...
        const color_styles = {
            option: (styles, { data, isSelected }) => {
                return {
                    ...styles,
                    color: data.value === "" ? 'grey' : isSelected ? 'white' : 'black'
                }
            },
        ...
        return (
            ...
            <p>To use the model, select a country, followed by a state if available. Not all states are available. If no state is selected, the model aggregates data for the selected country.</p>
            <div style={{ marginTop: 20, marginBottom: 20 }}>
            <h4>Country</h4>
            <Select 
            ...
            onChange={(selected) => this.setState({ country: selected.value })}
            />
            {!no_states && 
            <div>
